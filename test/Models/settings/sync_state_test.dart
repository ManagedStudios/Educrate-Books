  ...
  group('SyncState', () {
    ...
    group('init', () {
      test('sets status to noCredentials if username is missing', () async {
        ...
        when(() => mockStorage.read(key: 'sync_uri')).thenAnswer((_) async => 'url');
    ...
    group('saveCredentials', () {
      test('saves all credentials and restarts replication', () async {
        await syncState.saveCredentials( 'url', 'user', 'pass');
        ...
        verify(() => mockStorage.write(key: 'sync_uri', value: 'url')).called(1);
