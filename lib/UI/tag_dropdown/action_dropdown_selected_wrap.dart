...
class _ActionDropdownSelectedWrapState<T extends LfgChip> extends State<ActionDropdownSelectedWrap<T>> {
  ...
  Widget build(BuildContext context) {
    return Card( //base container providing shape and color
      ...
      child: SizedBox( //limit the width to match ActionDropdownAvailableContainer
        key: Key(Keys.ActionDropdownSelectedWrapSizedBoxForCard),
        width: widget.width,
        child: Wrap(
          crossAxisAlignment: WrapCrossAlignment.center,
          children: [
            for(int chipIndex = 0; chipIndex<(widget.selectedChips.length); chipIndex++)
              ChipTag(key: Key(widget.selectedChips[chipIndex].getLabelText()), chipContent: widget.selectedChips[chipIndex],
                  color: ChipColors.chipColors[chipIndex%ChipColors.chipColors.length],
                  deletable: true, onDelete: (chip) {
                    widget.onDeleteChip(chip); //delegate the onDeleteCall further up
                  }),
            /*
                ...
                child: Center(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      focusedBorder: InputBorder.none, // removes the border when the TextField is focused
                      enabledBorder: InputBorder.none, // removes the border when the TextField is enabled
                      errorBorder: InputBorder.none, // removes the border when the TextField has an error
                      disabledBorder: InputBorder.none, // removes the border when the TextField is disabled
