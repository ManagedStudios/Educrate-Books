...
class ActionDropdownAvailableContainer<T extends LfgChip>
    extends StatelessWidget {
  const ActionDropdownAvailableContainer(
      {super.key,
  ...
  Widget build(BuildContext context) {
    availableChips.sort();
    return Card(
      //base container providing color and shape
      ...
      child: SizedBox(
        //limiting the height of the AvailableContainer
        ...
        child: Padding(
          ...
          child: ListView(
            ...
            children: [
               ...
               */
              for (int chipIndex = 0;
                  chipIndex < availableChips.length;
                  chipIndex++)
                ChipWrap<T>(
                  chips: [availableChips[chipIndex]],
                  onClickChipRow: (List<T> chip) {
                    onAddChip(chip[0]);
                  },
                  width: width,
                  color: ChipColors.chipColors[ChipColors.chipColors.length -
                      1 -
                      chipIndex % ChipColors.chipColors.length],
                )
            ],
          ),
        ),
