...
class _StudentDetailBookListState extends State<StudentDetailBookList> {
  SplayTreeSet selectedBookIndices =
      SplayTreeSet(); //ordered indexed list to efficiently add and decide which books are selected
        ...
        for (int index = 0; index < widget.books.length; index++)
          BookCard(
              ...
              onClick: (book) => selectBooks(
                    //method to select multiple books
                    books: widget.books,
                    index: index,
    ...
    setState(() {
      switch (pressedKey) {
        case Keyboard.nothing:
          {
            //one book cannot be selected, but clears the old selection
            selectedBookIndices.clear();
            widget.onClearSelectedBooks();
          }
        case Keyboard.cmd:
